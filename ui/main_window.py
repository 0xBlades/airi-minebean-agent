...
class SignalBridge(QObject):
    ...
    minebean_grid_update = pyqtSignal(dict)
    minebean_round_start = pyqtSignal(dict)
    minebean_wallet_update = pyqtSignal(dict)
    minebean_ai_log = pyqtSignal(str)
    minebean_winrate = pyqtSignal(dict)
...
class AiriMainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.scroll_layout.addWidget(self._build_minebean_section())
    ...
    def _build_minebean_section(self) -> QWidget:
        """Build the Minebean wallet & stats section."""
        card = QWidget()
        ...
        self.beanpot_label = QLabel("Beanpot: ...")
        self.beanpot_label.setStyleSheet("color: #fbbf24; font-size: 10px; font-weight: 600;")
        header_row.addWidget(self.beanpot_label)
        ...
        self.eth_bal_label = QLabel("ETH: ...")
        self.eth_bal_label.setStyleSheet("color: #e2e8f0; font-size: 13px; font-weight: 700;")
        bal_layout.addWidget(self.eth_bal_label)

        bal_layout.addStretch()

        self.bean_bal_label = QLabel("BEAN: ...")
        self.bean_bal_label.setStyleSheet("color: #34d399; font-size: 13px; font-weight: 700;")
        bal_layout.addWidget(self.bean_bal_label)
    ...
    def _build_round_history_section(self) -> QWidget:
        ...
        # Pending ETH column
        eth_col = QVBoxLayout()
        eth_title = QLabel("Pending ETH")
        eth_title.setStyleSheet("color: #64748b; font-size: 9px;")
        eth_title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        eth_col.addWidget(eth_title)
        eth_col.addWidget(self.history_pnl)
        stats_layout.addLayout(eth_col)

        # Pending BEAN column
        bean_col = QVBoxLayout()
        bean_title = QLabel("Pending BEAN")
        bean_title.setStyleSheet("color: #64748b; font-size: 9px;")
        bean_title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.history_bean = QLabel("0.00")
        self.history_bean.setStyleSheet("color: #e2e8f0; font-size: 16px; font-weight: 800;")
        self.history_bean.setAlignment(Qt.AlignmentFlag.AlignCenter)
        bean_col.addWidget(bean_title)
        bean_col.addWidget(self.history_bean)
        stats_layout.addLayout(bean_col)
    ...
    def _build_agent_logs_section(self) -> QWidget:
        ...
        # Log text
        self.agent_log_label = QLabel("Menunggu koneksi ke Minebean...")
    ...
    def _connect_signals(self):
        ...
        self.signal_bridge.minebean_wallet_update.connect(self._update_wallet_ui)
        self.signal_bridge.minebean_round_start.connect(self._update_round_ui)
        self.signal_bridge.minebean_ai_log.connect(self._append_ai_log)
        self.signal_bridge.minebean_winrate.connect(self._update_winrate_ui)
    ...
    def signal_bridge_callback(self, event_type: str, data):
        ...
        elif event_type == "minebean_wallet_update":
            self.signal_bridge.minebean_wallet_update.emit(data)
        elif event_type == "minebean_round_start":
            self.signal_bridge.minebean_round_start.emit(data)
        elif event_type == "minebean_ai_log":
            self.signal_bridge.minebean_ai_log.emit(data)
        elif event_type == "minebean_winrate":
            self.signal_bridge.minebean_winrate.emit(data)
    ...
    def _update_wallet_ui(self, data: dict):
        ...
        self.wallet_addr_label.setText(f"Wallet: {short_addr}")

        eth_bal = data.get("eth_balance", 0.0)
    ...
    def _update_round_ui(self, data: dict):
        ...
        beanpot = data.get("beanpotPoolFormatted", "0.0")
        round_id = data.get("roundId", "?")
        self.beanpot_label.setText(f"R#{round_id} | Pot: {beanpot} BEAN")
